...
public class TestGauss {
	...
	public void test() {
		...
		for (int r = 0; r < rows; r++) {
			...
			zaehler = (random.nextInt(100) + 1) * (random.nextInt(1) == 1 ? 1 : -1);
			nenner = random.nextInt(100) + 1;
		...
		for (int i = 0; i < 25; i++) {
			MatrixOps.addRows(random.nextInt(rows), random.nextInt(rows), new Rational(
					(random.nextInt(5) + 1) * (random.nextInt(1) == 1 ? 1 : -1), random.nextInt(5) + 1), matrix);
		}
