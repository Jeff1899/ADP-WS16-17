...
public class Generator {
    public KFG generateKFG(Produktion startProduktionen, List<Produktion> restlicheProduktionen){

        return null;
    }

    public KFG generateLambdaFreeKFG(KFG kfg){
    ...
    public KFG generateChomskyKFG(KFG externKfg){
        ...
        for(int index = 0; index < kfg.getProduktionen().size(); index++){
            ...
            for(AbstractSymbol[] symbolFolge : produktion.getRumpf()){
                if (symbolFolge != null && symbolFolge.length > 2){
                    int j = 0;
